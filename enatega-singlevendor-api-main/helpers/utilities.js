...
const sendNotification = async order_id => {
  ...
  if (configuration.push_token) {
    var message = {
      data: {
        orderid: order_id
      },
      token: configuration.push_token,
      notification: {
    ...
    Food.findById(item.food.id)
      .then(food => {
        ...
        food
          ...
          .catch(e => {
            console.log('error while updating stock', e)
          })
      })
      .catch(e => {
        console.log('error while looking for food to update stock', e)
      })
  })
}
